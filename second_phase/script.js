...
function createCard(imageUrl, index) {
    ...
    const onEnd = () => {
        ...
        if (currentX > 50) {
            handleAction(index, 'rejected'); // Swiping right = rejecting
        } else if (currentX < -50) {
            handleAction(index, 'liked'); // Swiping left = accepting
        } else {
...
function updateStack() {
    const remainingCards = Array.from(document.querySelectorAll('.swipe-card:not([style*="translateX(-500px)"]):not([style*="translateX(500px)"])'))
...
document.getElementById('btnReject').addEventListener('click', () => {
    if (currentCardIndex < images.length) handleAction(currentCardIndex, 'rejected');
});
